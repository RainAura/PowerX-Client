...
public class PowerFont {
	...
	public PowerFont(final Font font, final boolean antiAlias, final int charOffset, final boolean allChar,
			final int yAddon) {
	...
	public PowerFont(final Font font, final boolean antiAlias, final boolean allChar, final int yAddon) {
		this.IMAGE_WIDTH = 1024;
		this.IMAGE_HEIGHT = 1024;
		this.chars = new IntObject[2048];
		this.fontHeight = -1;
	...
	private void setupTexture(final boolean antiAlias, final boolean allChar, final int yAddon) {
		...
		try {
			this.texID = TextureUtil.uploadTextureImageAllocate(TextureUtil.glGenTextures(), img, true, !allChar);
	...
	public void drawChar(final char c, final float x, final float y) throws ArrayIndexOutOfBoundsException {
		try {
			this.drawQuad(x, y, this.chars[c].width, this.chars[c].height, this.chars[c].storedX, this.chars[c].storedY,
