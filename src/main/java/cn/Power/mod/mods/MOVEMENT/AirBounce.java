...
public class AirBounce extends Mod {

	private Value<String> mode = new Value("AirBounce", "Mode", 0);
	public static Value<Boolean> OFF = new Value<Boolean>("AirBounce_Toggle", true);
	public static Value<Boolean> voidCheck = new Value<Boolean>("AirBounce_Void", true);
	private static Value<Double> BOOST = new Value<Double>("AirBounce_Motion", 1.9, 1.0, 10.0, 0.05);
	int stage = 0;
	Timer timer = new Timer();
	double speed, fall;
	boolean jumped,canSpeed = false;

	public AirBounce() {
		super("AirBounce", Category.MOVEMENT);
...
//				if (MathUtils.isOnGround(0.001) && mc.thePlayer.isCollidedVertically) {
...
//				}
    ...
    public static void setVanillaSpeed(final EventMove event, final double speed) {
...
//		if (mode.isCurrentMode("Hypixel")) {
			...
			if (MathUtils.isOnGround(0.0001) && mc.thePlayer.isCollidedVertically) {
				fall = 0;
				jumped = false;
	//			canSpeed = false;
			...
			double motion = BOOST.getValueState().doubleValue();
			if (mc.thePlayer.motionY < -0.08 && (!voidCheck.getValueState() || !this.isBlockUnder())) {
...
/*		}
			...
			if (!MathUtils.isOnGround(0.4)) {
				...
				if (stage != -999) {
					...
					if (mc.thePlayer.motionY < -0.2) {
						...
						double test = diff > val ? val - mc.thePlayer.fallDistance / 300
								: diff - mc.thePlayer.fallDistance / 1500;
						if (test < 0)
							test = 0;
				...
				stage = -999;
				if (mc.thePlayer.isCollidedVertically && MathUtils.isOnGround(0.001)) {
					if (OFF.getValueState().booleanValue()) {
						if (groundTicks >= 1) {
							groundTicks = 0;
							toggle();
						}
		...
		if (mode.isCurrentMode("Mineplex")) {
			...
			double dist = 0.1;
			double nextX = x + (forward * 0.45 * Math.cos(Math.toRadians(YAW + 90.0f))
					+ strafe * 0.45 * Math.sin(Math.toRadians(YAW + 90.0f))) * dist;
			double nextZ = z + (forward * 0.45 * Math.sin(Math.toRadians(YAW + 90.0f))
			...
			if (jumped) {
				if (!MathUtils.isOnGround(0.001)) {
					...
					if (speed < MathUtils.defaultSpeed())
						speed = MathUtils.defaultSpeed();
					MathUtils.setMotion(speed);
					stage++;
				} else {
					if (OFF.getValueState().booleanValue()) {
						toggle();
			...
			if (should && MathUtils.isOnGround(0.001)) {
				...
				if (a != 11) {
					...
					em.setX(nextX);
					em.setZ(nextZ);
					mc.thePlayer.motionY = BOOST.getValueState().doubleValue();
				}
			}
		}
...
//		}
	...
	public void onjump(EventJump ej) {
		if (mode.isCurrentMode("Basic")) {
			...
			ej.setMotionY(BOOST.getValueState().doubleValue());
...
//			if (timer.delay(2000)) {
//				timer.reset();
//				Client.instance.getNotificationManager().addNotification("HighJump Walk off an island to highjump.", Type.INFO);
//			}
...
//		}
	...
	public double getBestMineplexExploit(double x, double y, double z) {
		...
		do {
			...
			if ((!mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer, bb.offset(0, -1, 0)).isEmpty()
					...
					&& mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer, bb.offset(0, -0.5, 0)).isEmpty()
					&& yOff <= -4.5) || yOff <= -9) {
	...
	public boolean isInBlock(Vec3 vec) {
		...
		if (mc.theWorld.rayTraceBlocks(vec1, vec2) != null
				&& mc.theWorld.rayTraceBlocks(vec1, vec2).typeOfHit == MovingObjectType.BLOCK) {
		...
		if (mc.theWorld.rayTraceBlocks(vec3, vec4) != null
				&& mc.theWorld.rayTraceBlocks(vec3, vec4).typeOfHit == MovingObjectType.BLOCK) {
		...
		if (mc.theWorld.rayTraceBlocks(vec5, vec6) != null
				&& mc.theWorld.rayTraceBlocks(vec5, vec6).typeOfHit == MovingObjectType.BLOCK) {
		...
		if (mc.theWorld.rayTraceBlocks(vec7, vec8) != null
				&& mc.theWorld.rayTraceBlocks(vec7, vec8).typeOfHit == MovingObjectType.BLOCK) {
