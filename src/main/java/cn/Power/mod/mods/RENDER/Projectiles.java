...
public class Projectiles extends Mod {
	public Value<Boolean> arrowESP = new Value("Projectiles_ArrowESP", true);
	...
	public void arrowESP(EventRender event) {
		...
		if (mc.thePlayer.inventory.getCurrentItem() != null) {
			...
			if ((item == 261 || item == 368 || item == 332 || item == 344)) {
				...
				GL11.glColor4f(0.0F, 1.0F, 2.2F, 0.3F);
				GL11.glBegin(GL11.GL_LINE_STRIP);

				for (int i = 0; i < 2000; i++) {
					GL11.glVertex3d(posX - mc.getRenderManager().renderPosX, posY - mc.getRenderManager().renderPosY,
				...
				case 3:
					...
					aim = new AxisAlignedBB(0.2D, 0.5D, -0.8D, 0.8D, 0.5D, -0.2D);
					break;

				case 4:
				case 5:
					GlStateManager.rotate(90, 0, 0, 1);
					aim = new AxisAlignedBB(0.2D, -0.5D, 0.2D, 0.8D, -0.5D, 0.8D);
					break;

				default:
					aim = new AxisAlignedBB(0.2D, 0.5, 0.2D, 0.8D, 0.5D, 0.8D);
					break;
				}
				drawBox(aim);
	...
	public static void setColor1(int colorHex) {
