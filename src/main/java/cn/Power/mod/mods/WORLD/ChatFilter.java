...
public class ChatFilter extends Mod {
	public Value mode = new Value("ChatFilter", "ChatMode", 0);
	private int randbypass = 0;
	private boolean swith = true;

	public ChatFilter() {
		super("ChatFilter", Category.WORLD);
					...
					if (v1.startsWith("/")) {
						...
						if (Stream.of(array).anyMatch(v3::equalsIgnoreCase)) {
							if (Stream.of(new String[] { "/r", "/shout", "/ac", "/pc", "/gc" })
								...
								chat.message = v2[0] + " " + this.bypassMsg(
										String.join(" ", (CharSequence[]) Arrays.copyOfRange(v2, 1, v2.length)));
								if (a2 != 0) {
									break Label_0315;
								}
							}
							if (v2.length > 1) {
								chat.message = v2[0] + " " + v2[1] + " " + this.bypassMsg(
										String.join(" ", (CharSequence[]) Arrays.copyOfRange(v2, 2, v2.length)));
							}
						}
					}
				}
	...
	private String bypassMsg(String v1) {
		...
		for (char c : v1.toCharArray()) {
			if (c >= 33 && c <= 65535) {
				randbypass++;
				stringBuilder.append(c + (swith ? rand() : rand2()));
			} else {
...
//        		+"\u0887\u0895\u0898\u0899"
	...
	private String rand() {
		...
		case 10: {
			randbypass = new Random().nextInt(10);
			return "\u0383";
		}
		}
		if (randbypass > 11) {
