...
import cn.Power.auth.microsoft.LoginWithMicrosoftDialog;
...
public class GuiAltManager extends GuiScreen {
	...
	public static Runnable microsoft_loginthread = new LoginWithMicrosoftDialog();
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		if (Mouse.hasWheel() && AltManager.getAlts().size() >= 11) {
		...
		this.drawDefaultBackground();
		Helper.mc.fontRendererObj.drawStringWithShadow(GuiAltManager.mc.session.getUsername(), 8.0f, 10.0f, 0x33ffff);
		Client.instance.getAltManager();
		Helper.mc.fontRendererObj.drawCenteredString(AltManager.getAlts().size() + " alts", this.width / 2, 10, -1);
		...
		RenderUtil.rectangleBordered(50.0, 33.0, this.width - 50, this.height - 50, 1.0,
				Colors.getColor(25, 25, 25, 150), Colors.getColor(5, 5, 5, 255));
		GL11.glPushMatrix();
	...
	/*
	 * @Override public void initGui() { this.buttonList.add(new GuiButton(0,
	 * this.width / 2 + 4 + 76, this.height - 24, 75, 20, "Cancel")); this.login =
	 * new GuiButton(1, this.width / 2 - 154, this.height - 48, 70, 20, "Login");
	 * this.buttonList.add(this.login); this.remove = new GuiButton(2, this.width /
	 * 2 - 74, this.height - 24, 70, 20, "Remove");
	 * this.buttonList.add(this.remove); this.buttonList.add(new GuiButton(3,
	 * this.width / 2 + 4 + 76, this.height - 48, 75, 20, "Add"));
	 * this.buttonList.add(new GuiButton(4, this.width / 2 - 74, this.height - 48,
	 * 70, 20, "Direct Login")); this.buttonList.add(new GuiButton(5, this.width / 2
	 * + 4, this.height - 48, 70, 20, "Random")); this.rename = new GuiButton(6,
	 * this.width / 2 + 4, this.height - 24, 70, 20, "Edit");
	 * this.buttonList.add(this.rename); this.rename = new GuiButton(7, this.width /
	 * 2 - 154, this.height - 24, 70, 20, "Last Alt");
	 * this.buttonList.add(this.rename); this.login.enabled = false;
	 * this.remove.enabled = false; this.rename.enabled = false; }
	 */
	...
	public void initGui() {
		...
		this.buttonList.add(new GuiButton(7, this.width / 2 - 190, this.height - 48, 60, 20, "Last Alt"));
		this.buttonList.add(new GuiButton(8, this.width / 2 - 190, this.height - 24, 60, 20, "Multi"));
		this.buttonList.add(new GuiButton(9, this.width  - 88, 7, 60, 20, "Microsoft"));
