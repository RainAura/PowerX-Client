...
public class AStarCustomPathFinder {
	...
	private double minDistanceSquared = 2;
	private boolean nearest = true;

	private static Vec3[] flatCardinalDirections = { new Vec3(1, 0, 0), new Vec3(-1, 0, 0), new Vec3(0, 0, 1),
			new Vec3(0, 0, -1) };
	...
	public static boolean checkPositionValidity(Vec3 vec3) {
		BlockPos pos = new BlockPos(vec3.mc());
	...
	public boolean addHub(Hub parent, Vec3 loc, double cost) {
		...
		if (existingHub == null) {
			if ((loc.getX() == endVec3.getX() && loc.getY() == endVec3.getY() && loc.getZ() == endVec3.getZ())
					|| (minDistanceSquared != 0 && loc.squareDistanceTo(endVec3) <= minDistanceSquared)) {
				path.clear();
				path = parent.getPath();
				path.add(loc);
				return true;
	...
	private class Hub {
		...
		public Hub(Vec3 loc, Hub parent, ArrayList<Vec3> path, double squareDistanceToFromTarget, double cost,
				double totalCost) {
			this.loc = loc;
			this.parent = parent;
			this.path = path;
