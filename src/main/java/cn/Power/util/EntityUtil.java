...
public class EntityUtil {
	...
	public static float[] getRotationFromPosition(double x, double z, double y) {
		...
		float yaw = (float) ((double) ((float) (Math.atan2(zDiff, xDiff) * 180.0D / 3.141592653589793D) - 90.0F)
				+ Math.random() + Math.random() + Math.random() + Math.random() + Math.random() + Math.random());
		float pitch = (float) (-(Math.atan2(yDiff, dist) * 180.0D / 3.141592653589793D + Math.random()
				+ Math.random()));
	...
	public static Entity findClosestToCross(double range) {
		...
		for (Object o : Minecraft.getMinecraft().theWorld.loadedEntityList) {
			...
			if (ent != Minecraft.getMinecraft().thePlayer
					&& Minecraft.getMinecraft().thePlayer.getDistanceToEntity(ent) <= range
	...
	public static boolean isValid(boolean animals, boolean players, boolean invisibles, Entity e) {
		boolean state = false;
		if (!(!isMob(e) || isPlayer(e) || isAnimal(e) || isItem(e) || isProjectile(e) || isFalling(e)
				|| isInvisible(e))) {
			state = false;
		}
		if (!(!animals || !isAnimal(e) || isMob(e) || isPlayer(e) || isItem(e) || isProjectile(e) || isFalling(e)
				|| isInvisible(e))) {
			state = true;
		}
		if (!(!players || !isPlayer(e) || isMob(e) || isAnimal(e) || isItem(e) || isProjectile(e) || isFalling(e)
				|| isInvisible(e))) {
			state = true;
		}
		if (!(!isProjectile(e) || isMob(e) || isAnimal(e) || isPlayer(e) || isItem(e) || isFalling(e)
				|| isInvisible(e))) {
			state = false;
		}
		if (!(!isItem(e) || isMob(e) || isPlayer(e) || isProjectile(e) || isAnimal(e) || isFalling(e))) {
			state = false;
		}
		if (invisibles && isInvisible(e) && animals && isAnimal(e) && !isProjectile(e) && !isPlayer(e) && !isMob(e)) {
			state = true;
		}
		if (invisibles && isInvisible(e) && players && isPlayer(e) && !isProjectile(e) && !isAnimal(e) && !isMob(e)) {
			state = true;
		}
		if (invisibles && isInvisible(e) && isMob(e) && !isProjectile(e) && !isPlayer(e) && !isAnimal(e)) {
			state = false;
		}
		return state;
	}
	...
	public static double[] interpolate(final Entity entity) {
		final double partialTicks = Minecraft.getMinecraft().timer.renderPartialTicks;
		final double[] pos = { entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * partialTicks,
	...
	public static boolean isNotPlayer(Entity e) {
		if (!(isAnimal(e) || isMob(e) || isProjectile(e) || isFalling(e) || isItem(e))) {
			return false;
		}
		return true;
	}
