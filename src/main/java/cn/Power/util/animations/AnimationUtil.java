...
import cn.Power.util.animations.easings.Back;
import cn.Power.util.animations.easings.Bounce;
import cn.Power.util.animations.easings.Circ;
import cn.Power.util.animations.easings.Cubic;
import cn.Power.util.animations.easings.Elastic;
import cn.Power.util.animations.easings.Expo;
import cn.Power.util.animations.easings.Linear;
import cn.Power.util.animations.easings.Quad;
import cn.Power.util.animations.easings.Quart;
import cn.Power.util.animations.easings.Quint;
import cn.Power.util.animations.easings.Sine;
...
public class AnimationUtil {
	...
	public final double easeIn(Progression progression, double startValue, double endValue, double duration) {
		...
		if (progression.getValue() < duration)
			progression.setValue((progression.getValue() + (Client.delta * .001F)));
	...
	public static final double slide(double current, double min, double max, double speed, boolean sliding) {
		speed *= Client.delta * .2;
		return MathHelper.clamp_double(sliding ? current < max ? current + (max - current) * speed : current
				: current > min ? current - (current - min) * speed : current, min, max);
	}
