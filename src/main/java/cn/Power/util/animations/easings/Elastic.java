package cn.Power.util.animations.easings;

import cn.Power.util.animations.Easing;
...
public class Elastic extends Easing {
	...
	public static final double easeIn(double progression, double startValue, double endValue, double duration) {
		...
		final double s = p / 4;
		return -(a * (double) Math.pow(2, 10 * (progression -= 1))
				* (double) Math.sin((progression * duration - s) * (2 * (double) Math.PI) / p)) + startValue;
	}

	public static final double easeIn(double progression, double startValue, double endValue, double duration, double a,
			double p) {
		double s;
		if (progression == 0)
			return startValue;
		...
		return -(a * (double) Math.pow(2, 10 * (progression -= 1))
				* (double) Math.sin((progression * duration - s) * (2 * Math.PI) / p)) + startValue;
	}
	...
	public static final double easeOut(double progression, double startValue, double endValue, double duration) {
		...
		double a = endValue;
		double s = p / 4;
		return (a * (double) Math.pow(2, -10 * progression)
				* (double) Math.sin((progression * duration - s) * (2 * (double) Math.PI) / p) + endValue + startValue);
	}
	...
	public static final double easeInOut(double progression, double startValue, double endValue, double duration) {
		...
		if (progression < 1)
			return -.5f
					* (a * (double) Math.pow(2, 10 * (progression -= 1))
							* (double) Math.sin((progression * duration - s) * (2 * (double) Math.PI) / p))
					+ startValue;
		return a * (double) Math.pow(2, -10 * (progression -= 1))
				* (double) Math.sin((progression * duration - s) * (2 * (double) Math.PI) / p) * .5f + endValue
				+ startValue;
	}
