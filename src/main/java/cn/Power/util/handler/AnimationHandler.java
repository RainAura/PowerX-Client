...
public class AnimationHandler {
	public WeakHashMap<RenderChunk, AnimationData> timeStamps;
	public int mode = 4;
		...
		if (timeStamps.containsKey(renderChunk)) {
			...
			if (time == -1L) {
				...
				if (mode == 4) {
					...
					BlockPos zeroedCenteredChunkPos = renderChunk.getPosition().add(8,
							-renderChunk.getPosition().getY(), 8);
			...
			if (timeDif < animationDuration) {
				...
				if (mode == 2) {
					if (chunkY < Minecraft.getMinecraft().theWorld.provider.getCloudHeight()) {
						mode = 0;
					} else {
						mode = 1;
					}
				...
				case 0:
					...
					// GlStateManager.rotate(0, 0.0F, -chunkY + modY, 0.0F);
