...
public abstract class Shader {
	...
	public Shader(final String fragmentShader) {
		...
		try {
			final InputStream vertexStream = getClass()
					.getResourceAsStream("/assets/minecraft/Power/shader/vertex.vert");
			vertexShaderID = createShader(IOUtils.toString(vertexStream), ARBVertexShader.GL_VERTEX_SHADER_ARB);
			IOUtils.closeQuietly(vertexStream);

			final InputStream fragmentStream = getClass()
					.getResourceAsStream("/assets/minecraft/Power/shader/fragment/" + fragmentShader);
		...
		// ClientUtils.getLogger().info("[Shader] Successfully loaded: " +
		// fragmentShader);
	}
