...
//        try {
//            List<String> fileContent = FileUtil.read(FileUtil.WAYPOINTS);
//            for (String line : fileContent) {
//                String[] split = line.split(":");
//                String waypointName = split[0];
//                // name x y z color string
//                assert split.length == 6;
//                createWaypoint(waypointName, new Vec3(Double.parseDouble(split[1]), Double.parseDouble(split[2]), Double.parseDouble(split[3])), Integer.parseInt(split[4]));
//            }
//        } catch (Exception e) {
...
//            fileContent.add(String.format("%s:%s:%s:%s:%s", waypointName, x, y, z, waypoint.getColor()));
//        }
//        FileUtil.write(FileUtil.WAYPOINTS, fileContent, true);
//    }
	...
	public void createWaypoint(String name, Vec3 vec3, int color) {
		waypoints.add(new Waypoint(name, vec3, color));
//        saveWaypoints();
	}
