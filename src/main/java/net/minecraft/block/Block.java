...
public class Block {
	...
	public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
		if ((Helper.xray || Helper.xrayesp) && Helper.containsID(getIdFromBlock(this))) {
			if (!Helper.bypass)
				return true;
			...
			for (EnumFacing si : EnumFacing.VALUES) {
				...
				try {
					if (shouldSideBeRendered_(worldIn, pos.offset(si), si)) {

						if (Helper.UHCBypass) {
// 						return shouldSideBeRendered_(worldIn, pos,side); 
// 						return HorizontallyBlock(worldIn,pos) ? shouldSideBeRendered_(worldIn, pos,side) : true ;
							return true;
						} else {
							return shouldSideBeRendered_(worldIn, pos, side);
						}
	...
	public boolean HorizontallyBlock(IBlockAccess worldIn, BlockPos pos) {
		int i = (int) 120;
		if (CheckAir(worldIn, pos)) {
			return true;
		}
	...
	public boolean CheckAir(IBlockAccess worldIn, BlockPos pos) {
		if (CheckAir1(worldIn, pos) || CheckAir2(worldIn, pos) || CheckAir3(worldIn, pos) || CheckAir4(worldIn, pos)) {
			return true;
		}
		return false;
	}

	public boolean CheckAir1(IBlockAccess worldIn, BlockPos pos) {
		int i = (int) 50;
		while (i > 0) {
			BlockPos Pos = new BlockPos(pos.getX() + i, pos.getY(), pos.getZ());
	...
	public boolean CheckAir2(IBlockAccess worldIn, BlockPos pos) {
		int i = (int) 50;
		while (i > 0) {
			BlockPos Pos = new BlockPos(pos.getX() - i, pos.getY(), pos.getZ());
			if (!(worldIn.getBlockState(pos).getBlock() instanceof BlockAir)) {
	...
	public boolean CheckAir3(IBlockAccess worldIn, BlockPos pos) {
		int i = (int) 50;
		while (i > 0) {
			BlockPos Pos = new BlockPos(pos.getX(), pos.getY(), pos.getZ() + i);
			if (!(worldIn.getBlockState(pos).getBlock() instanceof BlockAir)) {
				return false;
			}
			--i;
	...
	public boolean CheckAir4(IBlockAccess worldIn, BlockPos pos) {
		int i = (int) 50;
		while (i > 0) {
			BlockPos Pos = new BlockPos(pos.getX() + i, pos.getY(), pos.getZ() - i);
			if (!(worldIn.getBlockState(pos).getBlock() instanceof BlockAir)) {
				return false;
			}
			--i;
	...
	public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask,
		...
		if (collidingEntity == Minecraft.thePlayer) {
			EventBlockBB e = new EventBlockBB(this, pos, axisalignedbb);
			EventManager.call(e);
	...
	public float getPlayerRelativeBlockHardness(EntityPlayer playerIn, World worldIn, BlockPos pos) {
		...
		if (f < 0.0F) {
			...
			return !playerIn.canHarvestBlock(this) ? playerIn.getToolDigEfficiency(this) / f / 100.0F
	...
	private boolean isVecInsideYZBounds(Vec3 point) {
		if (point == null) {
			...
			return point.yCoord >= this.minY && point.yCoord <= this.maxY && point.zCoord >= this.minZ
	...
	private boolean isVecInsideXZBounds(Vec3 point) {
		if (point == null) {
			...
			return point.xCoord >= this.minX && point.xCoord <= this.maxX && point.zCoord >= this.minZ
					&& point.zCoord <= this.maxZ;
		}
	}
	...
	private boolean isVecInsideXYBounds(Vec3 point) {
		if (point == null) {
			...
			return point.xCoord >= this.minX && point.xCoord <= this.maxX && point.yCoord >= this.minY
	...
	public EnumWorldBlockLayer getBlockLayer() {
		if (Helper.xray) {
			return Helper.containsID(getIdFromBlock(this)) ? EnumWorldBlockLayer.SOLID
