...
public class BlockDoor extends Block {
	...
	protected BlockDoor(Material materialIn) {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH)
				.withProperty(OPEN, false).withProperty(HINGE, BlockDoor.EnumHingePosition.LEFT)
				.withProperty(POWERED, false).withProperty(HALF, BlockDoor.EnumDoorHalf.LOWER));
	}
		...
		if (state.getValue(HALF) == BlockDoor.EnumDoorHalf.UPPER) {
			...
			if (flag1) {
				...
				if ((flag || neighborBlock.canProvidePower()) && neighborBlock != this
						&& flag != iblockstate1.getValue(POWERED)) {
