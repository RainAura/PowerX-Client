...
public class BlockFire extends Block {
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		if (!World.doesBlockHaveSolidTopSurface(worldIn, pos.down())
			...
			return state.withProperty(NORTH, this.canCatchFire(worldIn, pos.north()))
					...
					.withProperty(WEST, this.canCatchFire(worldIn, pos.west())).withProperty(UPPER, l)
					.withProperty(FLIP, flag1).withProperty(ALT, flag);
		} else {
	...
	protected BlockFire() {
		super(Material.fire);
		this.setDefaultState(this.blockState.getBaseState().withProperty(AGE, 0).withProperty(FLIP, false)
				.withProperty(ALT, false).withProperty(NORTH, false).withProperty(EAST, false)
				.withProperty(SOUTH, false).withProperty(WEST, false).withProperty(UPPER, 0));
