...
public abstract class BlockLiquid extends Block {
	...
	protected int getLevel(IBlockAccess worldIn, BlockPos pos) {
		return worldIn.getBlockState(pos).getBlock().getMaterial() == this.blockMaterial
				? worldIn.getBlockState(pos).getValue(LEVEL)
				: -1;
	}
	...
	public AxisAlignedBB getCollisionBoundingBox(World worldIn, BlockPos pos, IBlockState state) {
		EventLiquidCollide eb = new EventLiquidCollide();
		eb.fire(this, pos, new AxisAlignedBB(pos.getX() + this.minX, pos.getY() + this.minY, pos.getZ() + this.minZ,
				pos.getX() + this.maxX, pos.getY() + this.maxY, pos.getZ() + this.maxZ));
		EventManager.call(eb);
		if (eb.isCancelled()) {
			return eb.getBounds();
		}
		return null;
