...
import cn.Power.util.license.UnsafeUtils;
import cn.Power.util.license.VMManager;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
	...
	private static final ResourceLocation locationMojangPng = new ResourceLocation("textures/gui/title/Power.png");
	...
	private void startGame() throws LWJGLException, IOException {
		...
		this.loadScreen(this.renderEngine, 1, 1, "RenderEngine");
		// this.initStream();
		...
		this.loadScreen(this.renderEngine, 2, 2, "FontRender");
		...
		this.loadScreen(this.renderEngine, 3, 3, "Shader");
		...
		this.loadScreen(this.renderEngine, 4, 4, "Textures");
		...
		this.loadScreen(this.renderEngine, 4, 4, "Listeners");

		this.modelManager = new ModelManager(this.textureMapBlocks);
		this.mcResourceManager.registerReloadListener(this.modelManager);
		this.renderItem = new RenderItem(this.renderEngine, this.modelManager);

		this.loadScreen(this.renderEngine, 4, 6, "Engine");

		this.renderManager = new RenderManager(this.renderEngine, this.renderItem);
		this.itemRenderer = new ItemRenderer(this);
		this.mcResourceManager.registerReloadListener(this.renderItem);

		this.loadScreen(this.renderEngine, 5, 6, "Manager");
		...
		this.loadScreen(this.renderEngine, 6, 7, "Gui");
		...
		Client.instance.onClientStart();

		this.loadScreen(this.renderEngine, 6, 7, "Client");
...
//		}
		...
		this.loadScreen(this.renderEngine, 7, 6, "Finished");
	...
	private void initStream() {
		try {
			this.stream = new TwitchStream(this,
					Iterables.getFirst(this.twitchDetails.get("twitch_access_token"), (Property) null));
		} catch (Throwable throwable) {
		...
		if (util$enumos != Util.EnumOS.OSX) {
			...
			try {
				inputstream = this.mcDefaultResourcePack
						.getInputStream(new ResourceLocation("Power/icons/Power16.png"));
				inputstream1 = this.mcDefaultResourcePack
						.getInputStream(new ResourceLocation("Power/icons/Power32.png"));
	...
	public void loadScreen(TextureManager textureManagerInstance, final int step, final int progress,
		...
		GL11.glLoadIdentity();
		GL11.glOrtho(0.0, (double) scaledresolution.getScaledWidth(), (double) scaledresolution.getScaledHeight(), 0.0,
				1000.0, 3000.0);
		GL11.glMatrixMode(5888);
		GL11.glLoadIdentity();
		GL11.glTranslatef(0.0f, 0.0f, -2000.0f);

		mc.fontRendererObj.drawString(display,
				scaledresolution.getScaledWidth() / 2 - mc.fontRendererObj.getStringWidth(display) / 2, 200, 10000);
		double percent = step / 7.0;
		percent += progress / 100.0 / 7.0;
		mc.fontRendererObj.drawStringWithShadow("\247a" + (int) (percent * 100.0) + "%", scaledresolution.getScaledWidth() / 2
				- mc.fontRendererObj.getStringWidth((int) percent * 100 + "%") / 2, 220, 10000);
	...
	public void scaledTessellator(int scale) {
		...
		worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
		worldrenderer.pos(0.0D, 0.0D, 0.0D).tex(0.0D, 0.0D).color(255, 255, 255, 255).endVertex();
		worldrenderer.pos(0.0D, Display.getHeight(), 0.0D).tex(0, scale).color(255, 255, 255, 255).endVertex();
		worldrenderer.pos(Display.getWidth(), Display.getHeight(), 0.0D).tex(scale, scale).color(255, 255, 255, 255)
				.endVertex();
		worldrenderer.pos(Display.getWidth(), 0.0D, 0.0D).tex(scale, 0.0D).color(255, 255, 255, 255).endVertex();
	...
	private void runGameLoop() throws IOException {
		final long currentTime = (Sys.getTime() * 1000L) / Sys.getTimerResolution();
		final int deltaTime = (int) (currentTime - this.LastFrame);
		this.LastFrame = currentTime;
		if (deltaTime >= 0)
			Client.instance.delta = deltaTime;
		else
			Client.instance.delta = 0;
	...
	//	this.checkGLError("Post render");
		...
		if (getSystemTime() >= this.debugUpdateTime + 1000L) {
...
//		this.mcProfiler.endSection();
		...
		if (this.mcProfiler.profilingEnabled) {
			...
			for (int l = 0; l < list.size(); ++l) {
				...
				for (int j2 = i1; j2 >= 0; --j2) {
					float f = (float) ((d0 + profiler$result1.field_76332_a * (double) j2 / (double) i1)
							* (double) (float) Math.PI * 2.0D / 100.0D);
				...
				for (int i3 = i1; i3 >= 0; --i3) {
					float f3 = (float) ((d0 + profiler$result1.field_76332_a * (double) i3 / (double) i1)
							* (double) (float) Math.PI * 2.0D / 100.0D);
	...
	public void runTick() throws IOException {
		...
		EventTick even2t = new EventTick();
		EventManager.call(even2t);
...
//						System.err.println(k);
			...
			if (this.currentScreen == null && this.gameSettings.keyBindCommand.isPressed() && flag) {
				this.displayGuiScreen(new GuiChat("/"));
			} else if (this.currentScreen == null && this.gameSettings.keyBindClientCommand.isPressed() && flag) {
				this.displayGuiScreen(new GuiChat("-"));
			}
	...
	@native0
	public static String[] VerCheck(String url) {
		String result = "";
		String A = "";
		String B = "";
		try {
	...
	public MusicTicker.MusicType getAmbientMusicType() {
		if (this.thePlayer != null) {
			if (this.thePlayer.worldObj.provider instanceof WorldProviderHell) {
				...
				return BossStatus.bossName != null && BossStatus.statusBarTime > 0 ? MusicTicker.MusicType.END_BOSS
						: MusicTicker.MusicType.END;
			} else {
				return this.thePlayer.capabilities.isCreativeMode && this.thePlayer.capabilities.allowFlying
