...
public class GuiPlayerTabOverlay extends Gui {
	...
	public String getPlayerName(NetworkPlayerInfo networkPlayerInfoIn) {
        ...
        if (networkPlayerInfoIn.getDisplayName() != null) {
            result = networkPlayerInfoIn.getDisplayName().getFormattedText();
        } else {
            result = ScorePlayerTeam.formatPlayerName(networkPlayerInfoIn.getPlayerTeam(),networkPlayerInfoIn.getGameProfile().getName());
        }   
        if (Objects.nonNull(Client.instance.IRC))
            if (!Client.instance.IRC.userlist.isEmpty() && Client.instance.IRC.user.getRankLevel() >= 3) {
                for (User user : Client.instance.IRC.userlist) {
                    if (networkPlayerInfoIn.getGameProfile().getName().equals(user.gameID)) {
                        if (user.hide) {
                            if (Client.instance.IRC.user.getRankLevel() >= 5) {
                                if (user.client.equalsIgnoreCase("FoodByte")) {
                                    result = result.replace(user.gameID, user.gameID + "\2477[\247d\247o" + user.name + "\2477]\247r".replace(user.name,"\247o"+user.name));
                                } else if (user.client.equalsIgnoreCase("PowerX")) {
                                    result = result.replace(user.gameID, user.gameID + "\2477[\247b\247o" + user.name + "\2477]\247r".replace(user.name,"\247o"+user.name));
                                } else {
                                    result = result.replace(user.gameID, user.gameID + "\2477[\2472\247o" + user.name + "\2477]\247r".replace(user.name,"\247o"+user.name));
                                }
                            }
                        } else {
                            if (user.client.equalsIgnoreCase("FoodByte")) {
                                result = result.replace(user.gameID, user.gameID + "\2477(\247d" + user.name + "\2477)\247r");
                            } else if (user.client.equalsIgnoreCase("PowerX")) {
                                result = result.replace(user.gameID, user.gameID + "\2477(\247b" + user.name + "\2477)\247r");
                            } else {
                                result = result.replace(user.gameID, user.gameID + "\2477(\2472" + user.name + "\2477)\247r");
                            }
                        }
                    }
                }
		...
		for (int k4 = 0; k4 < l3; ++k4) {
			...
			if (k4 < list.size()) {
				...
				if (networkplayerinfo1.getGameType() == WorldSettings.GameType.SPECTATOR) {
					...
					this.mc.fontRendererObj.drawStringWithShadow(s1.replace("§k", ""), (float) j2, (float) k2, -1862270977);
				} else {
					this.mc.fontRendererObj.drawStringWithShadow(s1.replace("§k", ""), (float) j2, (float) k2, -1);
				}
