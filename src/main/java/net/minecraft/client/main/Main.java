...
public class Main {
	...
	public static void loadVia(File mcdir) throws NoSuchFieldException, IllegalAccessException, MalformedURLException {
        ClassLoader loader = ClassLoader.getSystemClassLoader();
        Field addUrl = loader.getClass().getDeclaredField("ucp");
        addUrl.setAccessible(true);
        URLClassPath ucp = (URLClassPath) addUrl.get(loader);
        File file = new File(mcdir, "/Power/mods");
        final File[] files = file.listFiles();
        if(!file.exists() || !file.isDirectory()) {
        	JFrame jf=new JFrame();
        	jf.setAlwaysOnTop(true);
        	
        	JOptionPane.showConfirmDialog(jf, file.getAbsolutePath() + " No Found 没有找到。。。", "Power mods加载", JOptionPane.YES_OPTION);
        ...
        if(i == 0) {
         	JOptionPane.showConfirmDialog(jf, file.getAbsolutePath() + " ViaVersion Load Failed. 加载失败。。。", "Power mods加载", JOptionPane.ERROR_MESSAGE);
        }
