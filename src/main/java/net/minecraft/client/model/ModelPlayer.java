...
public class ModelPlayer extends ModelBiped {
	...
	public static ModelRenderer wing;
	public static ModelRenderer wingTip;
	boolean flying;
	private static DragWings dragonwin;

	private static final ResourceLocation texture = new ResourceLocation("Power/wings.png");
	...
	public void render(Entity entityIn, float p_78088_2_, float p_78088_3_, float p_78088_4_, float p_78088_5_, float p_78088_6_, float scale) {
		...
		if (this.isChild) {
			...
			if(dragonwin == null) {
				dragonwin = (DragWings) ModManager.getModByClass(DragWings.class);
			}
			
			if (dragonwin.isEnabled()
					&& Minecraft.getMinecraft().getNetHandler().getPlayerInfo(entityIn.getUniqueID()) == Minecraft
							.getMinecraft().getNetHandler()
							.getPlayerInfo(Minecraft.getMinecraft().thePlayer.getUniqueID())) {
				if (!entityIn.isInvisible()) {
					this.renderWings((EntityPlayer) entityIn, Minecraft.getMinecraft().timer.renderPartialTicks,
							p_78088_3_, p_78088_4_, scale);
				}
			}
			if (dragonwin.isEnabled() && DragWings.team.getValueState()
					&& Teams.isOnSameTeam(entityIn)) {
				this.renderWings((EntityPlayer) entityIn, Minecraft.getMinecraft().timer.renderPartialTicks, p_78088_3_,
						p_78088_4_, scale);
			}
		}
	...
	private void renderWings(EntityPlayer player, float partialTicks, float p_78088_3_, float p_78088_4_, float scale) {
		...
		float f4 = 0.125F - (float) Math.cos((double) f3) * 0.2F;
		float fs5 = f2 * 3.1415927F * 2.0F;
		float f5 = 0.125F - (float) Math.cos((double) fs5) * 0.2F;
		if (!player.onGround && (int) (f4 * 100.0F) == (int) (f5 * 100.0F)) {
			anSpeed = 100.0F;
		}

		GlStateManager.scale(0.7D, 0.7D, 0.7D);
		GlStateManager.translate(0.0D, 0.05000000074505806D, 0.18000000715255737D);
		GlStateManager.rotate(50.0F, -50.0F, 0.0F, 0.0F);
		float[] colors = new float[] { DragWings.r.getValueState().floatValue() / 255.0F,
				DragWings.g.getValueState().floatValue() / 255.0F, DragWings.b.getValueState().floatValue() / 255.0F };
	...
	public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
		...
		if (entityIn != Minecraft.thePlayer && entityIn instanceof EntityPlayer) {
