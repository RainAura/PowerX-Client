...
public class PlayerControllerMP {
	...
	public float getBlockReachDistance() {
		boolean in = ModManager.getModByClass(ReachBlock.class).isEnabled();
		return in ? 60 : this.currentGameType.isCreative() ? 5.0F : 4.5F;
	}
	...
	public void syncCurrentPlayItem() {
		...
		if (i < 0 || i >= 9)
			this.mc.thePlayer.inventory.currentItem = i = 0;
			...
			if (this.currentGameType != WorldSettings.GameType.SPECTATOR) {
				...
				if (!flag && heldStack != null && heldStack.getItem() instanceof ItemBlock) {
					...
					if (ViaLoadingBase.getInstance().getTargetVersion().getOriginalVersion() != 47)
						Minecraft.theWorld.playSound(hitVec.getX(), hitVec.getY(), hitVec.getZ(), "dig."
								+ (itemblock.getUnlocalizedName().toLowerCase().contains("wood") ? "wood" : "stone"),
								(float) 5.8 + new Random().nextFloat() / 10,
								(float) 1.0 - new Random().nextFloat() / 10, true);
				}
			}
	...
	public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn) {
		if (this.currentGameType == WorldSettings.GameType.SPECTATOR) {
			...
			if (playerIn.inventory.getCurrentItem().getItem() instanceof ItemSkull)
				this.netClientHandler.addToSendQueue(new C08PacketPlayerBlockPlacement(
						new BlockPos(mc.thePlayer.posX, mc.thePlayer.posY - 3, mc.thePlayer.posZ),
						EnumFacing.UP.getIndex(), playerIn.inventory.getCurrentItem(), 0.545678f, 1.0f, 0.7654321f));
	...
	public static int lastWindowsId;
...
//		System.out.println(mode + "  " + mouseButtonClicked);

		ItemStack itemstack = (windowId == 0 ? playerIn.inventoryContainer : playerIn.openContainer).slotClick(slotId,
				mouseButtonClicked, mode, playerIn);
		this.netClientHandler.getNetworkManager()
				.sendPacketNoEvent(new C0EPacketClickWindow(windowId, slotId, mouseButtonClicked, mode, itemstack,
						(short) (windowId == 0 ? playerIn.inventoryContainer : playerIn.openContainer)
								.getNextTransactionID(playerIn.inventory)));

//		if(windowId == 0 && playerIn == mc.thePlayer && !(mc.currentScreen instanceof GuiInventory))
//			mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C0DPacketCloseWindow(0));
...
//	 * Handles slot clicks sends a packet to the server.
//	 */
//	public ItemStack inventoryContainerwindowClick(int windowId, int slotId, int mouseButtonClicked, int mode, EntityPlayer playerIn) {
//		ItemStack itemstack = playerIn.inventoryContainer.slotClick(slotId, mouseButtonClicked, mode, playerIn);
//		this.netClientHandler.getNetworkManager()
//				.sendPacketNoEvent(new C0EPacketClickWindow(0, slotId, mouseButtonClicked, mode, itemstack,
//						(short) playerIn.inventoryContainer.getNextTransactionID(playerIn.inventory)));
//		
////		if(playerIn == mc.thePlayer && !(mc.currentScreen instanceof GuiInventory))
////			mc.getNetHandler().getNetworkManager().sendPacketNoEvent(new C0DPacketCloseWindow(0));
//		
//		return itemstack;
//	}
