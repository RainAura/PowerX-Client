...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
	...
	public void handleJoinGame(S01PacketJoinGame packetIn) {
		...
		this.netManager.sendPacket(new C17PacketCustomPayload("MC|Brand", p.writeString("vanilla")));
	...
	public static byte[] createPacket26() {
		...
		UUID uuid = Minecraft.thePlayer.getUniqueID();
	...
	public void handleSpawnObject(S0EPacketSpawnObject packetIn) {
		...
		if (packetIn.getType() == 10) {
			...
			entity = new EntityLargeFireball(this.clientWorldController, d0, d1, d2, packetIn.getSpeedX() / 8000.0D,
					packetIn.getSpeedY() / 8000.0D, packetIn.getSpeedZ() / 8000.0D);
			packetIn.func_149002_g(0);
		} else if (packetIn.getType() == 64) {
			entity = new EntitySmallFireball(this.clientWorldController, d0, d1, d2, packetIn.getSpeedX() / 8000.0D,
					packetIn.getSpeedY() / 8000.0D, packetIn.getSpeedZ() / 8000.0D);
			packetIn.func_149002_g(0);
		} else if (packetIn.getType() == 66) {
			entity = new EntityWitherSkull(this.clientWorldController, d0, d1, d2, packetIn.getSpeedX() / 8000.0D,
					packetIn.getSpeedY() / 8000.0D, packetIn.getSpeedZ() / 8000.0D);
			packetIn.func_149002_g(0);
		...
		if (entity != null) {
			...
			if (packetIn.func_149009_m() > 0) {
				...
				entity.setVelocity(packetIn.getSpeedX() / 8000.0D, packetIn.getSpeedY() / 8000.0D,
						packetIn.getSpeedZ() / 8000.0D);
			}
		}
	}
	...
	public void handleSpawnExperienceOrb(S11PacketSpawnExperienceOrb packetIn) {
		PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
		Entity entity = new EntityXPOrb(this.clientWorldController, packetIn.getX() / 32.0D, packetIn.getY() / 32.0D,
				packetIn.getZ() / 32.0D, packetIn.getXPValue());
		entity.serverPosX = packetIn.getX();
		entity.serverPosY = packetIn.getY();
		entity.serverPosZ = packetIn.getZ();
	...
	public void handleEntityVelocity(S12PacketEntityVelocity packetIn) {
		...
		if (entity != null) {
			entity.setVelocity(packetIn.getMotionX() / 8000.0D, packetIn.getMotionY() / 8000.0D,
					packetIn.getMotionZ() / 8000.0D);
		}
	}
	...
	public void handleSpawnPlayer(S0CPacketSpawnPlayer packetIn) {
		...
		if (packetIn.getPlayer() == null || this.getPlayerInfo(packetIn.getPlayer()) == null
		...
		try {
			...
			EntityOtherPlayerMP entityotherplayermp = new EntityOtherPlayerMP(Minecraft.theWorld,
	...
	public void handleDestroyEntities(S13PacketDestroyEntities packetIn) {
		...
		for (int i = 0; i < packetIn.getEntityIDs().length; ++i) {
			if (packetIn.getEntityIDs()[i] != -1
					&& Minecraft.theWorld.getEntityByID(packetIn.getEntityIDs()[i]) != null)
				this.clientWorldController.removeEntityFromWorld(packetIn.getEntityIDs()[i]);
		}
	}

	public int S08count = 0;
...
//		boolean set = false;
//
//		if (Minecraft.getMinecraft().ingameGUI != null && !Minecraft.getMinecraft().ingameGUI.displayedTitle.isEmpty() && Minecraft.getMinecraft().ingameGUI.displayedTitle.contains("SYNCING WORLD")) {
//			Disabler dis = (Disabler) ModManager.getModByClass(Disabler.class);
//			if (dis.isEnabled() && Disabler.mode.isCurrentMode("Hypixel")) {
//				if (S08count == 1) {
//					realPos = new double[] { d00, d10, d20 };
//					ChatUtil.printChat("Viaversion fix is On!");
//				}
//				if (S08count > 1 && realPos != null && Math.abs(Math.abs(d10) - Math.abs(realPos[1])) > 0.001) {
//					entityplayer.setPosition(realPos[0], realPos[1], realPos[2]);
//					set = true;
//				}
//				if (S08count < 20)
//					S08count++;
...
//
//		if(!set)
//			entityplayer.setPosition(d00, d10, d20);
//
//
//		if (!this.doneLoadingTerrain) {
...
//	}
	...
	public void dohandlePlayerPosLook(S08PacketPlayerPosLook packetIn)
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        EntityPlayer entityplayer = Minecraft.thePlayer;
        double d0 = packetIn.getX();
	...
	public void handleSpawnMob(S0FPacketSpawnMob packetIn) {
		...
		try {
			...
			Minecraft.theWorld.removeEntity(entitylivingbase);
		...
		if (packetIn.getDimensionID() != Minecraft.thePlayer.dimension) {
			...
			this.clientWorldController = new WorldClient(this,
					new WorldSettings(0L, packetIn.getGameType(), false,
							Minecraft.theWorld.getWorldInfo().isHardcoreModeEnabled(), packetIn.getWorldType()),
	...
	public void handleExplosion(S27PacketExplosion packetIn) {
		...
		if (packetIn.getStrength() > 0.0f) {
			Explosion explosion = new Explosion(Minecraft.theWorld, (Entity) null, packetIn.getX(), packetIn.getY(),
					packetIn.getZ(), packetIn.getStrength(), packetIn.getAffectedBlockPositions());
			explosion.doExplosionB(true);
		}

		Minecraft.thePlayer.motionX += packetIn.func_149149_c();
		Minecraft.thePlayer.motionY += packetIn.func_149144_d();
		Minecraft.thePlayer.motionZ += packetIn.func_149147_e();
	}
	...
	public void handleConfirmTransaction(S32PacketConfirmTransaction packetIn) {
		...
		// System.out.println(packetIn.getWindowId() + " " + packetIn.getActionNumber()
		// + " " + packetIn.func_148888_e());

		if (container != null && !packetIn.func_148888_e()) {
			this.getNetworkManager().sendPacket(
	...
	public void handleUpdateSign(S33PacketUpdateSign packetIn) {
		...
		if (!flag && Minecraft.thePlayer != null) {
			Minecraft.thePlayer.addChatMessage(new ChatComponentText("Unable to locate sign at "
	...
	public void handleBlockAction(S24PacketBlockAction packetIn) {
		PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
		Minecraft.theWorld.addBlockEvent(packetIn.getBlockPosition(), packetIn.getBlockType(), packetIn.getData1(),
				packetIn.getData2());
	}
	...
	public void handleBlockBreakAnim(S25PacketBlockBreakAnim packetIn) {
		PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
		Minecraft.theWorld.sendBlockBreakProgress(packetIn.getBreakerId(), packetIn.getPosition(),
	...
	public void handleChangeGameState(S2BPacketChangeGameState packetIn) {
		...
		if (i == 1) {
			...
			this.clientWorldController.playSound(entityplayer.posX, entityplayer.posY + entityplayer.getEyeHeight(),
	...
	public void handleEffect(S28PacketEffect packetIn) {
		...
		if (packetIn.isSoundServerwide()) {
			Minecraft.theWorld.playBroadcastSound(packetIn.getSoundType(), packetIn.getSoundPos(),
		...
		for (Entry<StatBase, Integer> entry : packetIn.func_148974_c().entrySet()) {
			...
			if (statbase.isAchievement() && i > 0) {
				if (this.field_147308_k && Minecraft.thePlayer.getStatFileWriter().readStat(statbase) == 0) {
					...
					// this.gameController.getTwitchStream().func_152911_a(new
					// MetadataAchievement(achievement), 0L);
...
//                this.gameController.getTwitchStream().func_152911_a(metadataplayerdeath, 0L);
	...
	public void handlePlayerListHeaderFooter(S47PacketPlayerListHeaderFooter packetIn) {
		...
		// System.out.println(IChatComponent.Serializer.componentToJson(packetIn.getHeader()));

		// TextComponent{text='', siblings=[TextComponent{text='You are playing on ',
		// siblings=[], style=Style{hasParent=true, color=§b, bold=null, italic=null,
		// underlined=null, obfuscated=null, clickEvent=null, hoverEvent=null,
		// insertion=null}}, TextComponent{text='MC.HYPIXEL.NET', siblings=[],
		// style=Style{hasParent=true, color=§e, bold=true, italic=null,
		// underlined=null, obfuscated=null, clickEvent=ClickEvent{action=OPEN_URL,
		// value='http://MC.HYPIXEL.NET'}, hoverEvent=null, insertion=null}}],
		// style=Style{hasParent=false, color=null, bold=null, italic=null,
		// underlined=null, obfuscated=null, clickEvent=null, hoverEvent=null,
		// insertion=null}}
		// TextComponent{text='', siblings=[TextComponent{text='Ranks, Boosters & MORE!
		// ', siblings=[], style=Style{hasParent=true, color=§a, bold=null, italic=null,
		// underlined=null, obfuscated=null, clickEvent=null, hoverEvent=null,
		// insertion=null}}, TextComponent{text='STORE.HYPIXEL.NET', siblings=[],
		// style=Style{hasParent=true, color=§c, bold=true, italic=null,
		// underlined=null, obfuscated=null, clickEvent=ClickEvent{action=OPEN_URL,
		// value='http://STORE.HYPIXEL.NET'}, hoverEvent=null, insertion=null}}],
		// style=Style{hasParent=false, color=null, bold=null, italic=null,
		// underlined=null, obfuscated=null, clickEvent=null, hoverEvent=null,
		// insertion=null}}

		// System.out.println(IChatComponent.Serializer.componentToJson(packetIn.getFooter()));
	...
	public void handleKeepAlive(S00PacketKeepAlive packetIn) {
		this.getNetworkManager().sendPacket(new C00PacketKeepAlive(packetIn.func_149134_c()));
	}
	...
	public void handlePlayerAbilities(S39PacketPlayerAbilities packetIn) {
		...
		System.out.println(packetIn.getWalkSpeed() + " " + packetIn.getFlySpeed() + " " + packetIn.isAllowFlying() + " "
				+ packetIn.isFlying() + " " + packetIn.isInvulnerable());
	}
	...
	public void handleSoundEffect(S29PacketSoundEffect packetIn) {
		PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
		if (packetIn.getSoundName().isEmpty() || packetIn.getSoundName().equals("minecraft:"))
			return;

		Minecraft.theWorld.playSound(packetIn.getX(), packetIn.getY(), packetIn.getZ(), packetIn.getSoundName(),
		...
		if ("MC|TrList".equals(packetIn.getChannelName())) {
			...
			try {
				...
				if (guiscreen != null && guiscreen instanceof GuiMerchant
						&& i == Minecraft.thePlayer.openContainer.windowId) {
			...
			Minecraft.thePlayer.setClientBrand(packetIn.getBufferData().readStringFromBuffer(327));

			System.out.println(Minecraft.thePlayer.getClientBrand());
	...
	public void handleScoreboardObjective(S3BPacketScoreboardObjective packetIn) {
		...
		if (packetIn.func_149338_e() == 0) {
			...
			// System.out.println(packetIn.func_149337_d() + " " +
			// packetIn.func_179817_d());
	...
	public void handleTeams(S3EPacketTeams packetIn) {
		...
		if (packetIn.getName().startsWith("team") && packetIn.getPrefix().contains("/")
				&& packetIn.getPrefix().startsWith("§7") && packetIn.getPrefix().contains("  §8")
				&& packetIn.getSuffix().startsWith("§8")) {
...
//		System.out.println("====================");
//		
//		System.out.println(packetIn.getName());
//		System.out.println(packetIn.getDisplayName());
//		System.out.println(packetIn.getPrefix());
//		System.out.println(packetIn.getSuffix());
//		System.out.println(packetIn.getNameTagVisibility());
//		System.out.println(packetIn.getColor());
//		packetIn.getPlayers().forEach(System.out::println);
//		System.out.println(packetIn.getAction());
//		System.out.println(packetIn.getFriendlyFlags());
//		
//		System.out.println("\t");
//		System.out.println("====================");
				...
				for (S20PacketEntityProperties.Snapshot s20packetentityproperties$snapshot : packetIn.func_149441_d()) {
					...
					if (iattributeinstance == null) {
						iattributeinstance = baseattributemap.registerAttribute(new RangedAttribute((IAttribute) null,
								s20packetentityproperties$snapshot.func_151409_a(), 0.0D, Double.MIN_NORMAL,
								Double.MAX_VALUE));
					}
