...
import cn.Power.mod.mods.PLAYER.NoHitBox;
...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	public void getMouseOver(float partialTicks) {
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			final Reach reach = (Reach) ModManager.getModByClass(Reach.class);
			boolean in = ModManager.getModByClass(ReachBlock.class).isEnabled();
			double d0 = (reach.isEnabled() && !mc.playerController.extendedReach()) ? reach.getMaxRange()
					: (double) this.mc.playerController.getBlockReachDistance();
			this.mc.objectMouseOver = entity.rayTrace((reach.isEnabled() && !mc.playerController.extendedReach())
					? in ? 60 : reach.BlockReach.getValueState()
					: d0, partialTicks);
			...
			if (this.mc.playerController.extendedReach()) {
				d0 = 6.0D;
				d1 = 6.0D;
			}  else if (d0 > ((ViaLoadingBase.getInstance().getTargetVersion().getIndex() >= 35) ? 3.0 : 2.9)) {
				flag = true;
			}
			...
			for (int j = 0; j < list.size(); ++j) {
				...
				Hitbox hitbox = (Hitbox) ModManager.getModByClass(Hitbox.class);
				float f1 = (float) ((hitbox.isEnabled() && hitbox.isvalid(entity1)) ? hitbox.size.getValueState().floatValue()
						: entity1.getCollisionBorderSize());
...
//			if (this.pointedEntity != null && flag && (vec3.distanceTo(vec33) > 3.1D && !reach.isEnabled())) {
//				this.pointedEntity = null;
//				this.mc.objectMouseOver = new MovingObjectPosition(MovingObjectType.MISS, vec33, (EnumFacing) null,
//						new BlockPos(vec33));
//			}

			if (this.pointedEntity != null && flag && vec33 != null && vec3.distanceTo(vec33) > ((ViaLoadingBase.getInstance().getTargetVersion().getIndex() >= 35) ? 3.0 : 2.9)) {
				this.pointedEntity = null;
			...
			if (this.pointedEntity != null && (d2 < d1 || this.mc.objectMouseOver == null) && (!(this.pointedEntity instanceof EntityPlayer) || !NoHitBox.status)) {
	...
	private void hurtCameraEffect(float partialTicks) {
		if (this.mc.getRenderViewEntity() instanceof EntityLivingBase
				&& !ModManager.getModByClass(NoHurtcam.class).isEnabled()) {
	...
	public void orientCamera(float partialTicks) {
		...
		if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
			...
			if (this.mc.gameSettings.debugCamEnable) {
				...
				if (!ModManager.getModByClass(ViewClip.class).isEnabled()) {
					for (int i = 0; i < 8; ++i) {
					...
					d3 = ViewClip.distance.getValueState().doubleValue();
		...
		if (world != null) {
			if (Config.getNewRelease() != null) {
				...
				chatcomponenttext.setChatStyle((new ChatStyle())
						.setChatClickEvent(new ClickEvent(Action.OPEN_URL, "https://optifine.net/downloads")));
