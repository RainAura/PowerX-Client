...
public class WorldRenderer
{
    ...
    public WorldRenderer(final int bufferSizeIn) {
        ...
        this.setRawShortBuffer(this.byteBuffer.asShortBuffer());
    ...
    private void growBuffer(final int p_181670_1_) {
        if (p_181670_1_ > this.rawIntBuffer.remaining()) {
            ...
            (this.setRawShortBuffer(this.byteBuffer.asShortBuffer())).position(l << 1);
    ...
    public void sortVertexData(final float p_181674_1_, final float p_181674_2_, final float p_181674_3_) {
        ...
        if (this.quadSprites != null) {
            final TextureAtlasSprite[] quadSpritesSorted = new TextureAtlasSprite[this.vertexCount / 4];
            for (int ix = 0; ix < ainteger.length; ++ix) {
                final int indexQuad = ainteger[ix];
                final int indexQuadSorted = ix;
    ...
    public State getVertexState() {
        ...
        return new State(aint, new VertexFormat(this.vertexFormat), quadSpritesCopy);
    }
    ...
    public void setVertexState(final State state) {
        ...
        if (state.stateQuadSprites != null) {
            ...
            final TextureAtlasSprite[] src = state.stateQuadSprites;
    ...
    public void putColorMultiplier(float red, float green, float blue, int p_178978_4_) {
		...
		if (Helper.xray) { 
			j = getColor(k, l, i1, Helper.opacity);
		}
    ...
    private void draw(final int p_draw_1_, final int p_draw_2_) {
        final int vxQuadCount = p_draw_2_ - p_draw_1_;
        if (vxQuadCount <= 0) {
            return;
        }
        final int startVertex = p_draw_1_ * 4;
    ...
    private int getBufferQuadSize() {
        final int quadSize = this.rawIntBuffer.capacity() * 4 / (this.vertexFormat.getIntegerSize() * 4);
        return quadSize;
    }
    ...
    public void putColorMultiplierRgba(final float p_putColorMultiplierRgba_1_, final float p_putColorMultiplierRgba_2_, final float p_putColorMultiplierRgba_3_, final float p_putColorMultiplierRgba_4_, final int p_putColorMultiplierRgba_5_) {
        final int index = this.getColorIndex(p_putColorMultiplierRgba_5_);
        int col = -1;
        if (!this.noColor) {
            col = this.rawIntBuffer.get(index);
            if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) {
                final int r = (int)((col & 0xFF) * p_putColorMultiplierRgba_1_);
                final int g = (int)((col >> 8 & 0xFF) * p_putColorMultiplierRgba_2_);
                final int b = (int)((col >> 16 & 0xFF) * p_putColorMultiplierRgba_3_);
                final int a = (int)((col >> 24 & 0xFF) * p_putColorMultiplierRgba_4_);
                col = (a << 24 | b << 16 | g << 8 | r);
            }
            else {
                final int r = (int)((col >> 24 & 0xFF) * p_putColorMultiplierRgba_1_);
                final int g = (int)((col >> 16 & 0xFF) * p_putColorMultiplierRgba_2_);
                final int b = (int)((col >> 8 & 0xFF) * p_putColorMultiplierRgba_3_);
                final int a = (int)((col & 0xFF) * p_putColorMultiplierRgba_4_);
                col = (r << 24 | g << 16 | b << 8 | a);
            }
        }
    ...
    public void quadsToTriangles() {
        ...
        final int vertexSize = this.vertexFormat.getNextOffset();
    ...
    public ShortBuffer getRawShortBuffer() {
		return rawShortBuffer;
	}

	public ShortBuffer setRawShortBuffer(ShortBuffer rawShortBuffer) {
		this.rawShortBuffer = rawShortBuffer;
		return rawShortBuffer;
	}
