...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    public static boolean notRenderingEffectsInGUI = true;
    ...
    private void renderEffect(IBakedModel model)
    {
        if (!Config.isCustomItems() || CustomItems.isUseGlint())
        {
            if (!Config.isShaders() || !Shaders.isShadowPass)
            {
				...
				if (ModManager.getModByClass(EnchantEffect.class).isEnabled()) {
					this.renderModel(model,
							EnchantEffect.Rainbow.getValueState().booleanValue() ? RenderUtil.rainbow(200)
									: new Color(EnchantEffect.r.getValueState().intValue(),
											EnchantEffect.g.getValueState().intValue(),
											EnchantEffect.b.getValueState().intValue()).getRGB());
				} else {
