...
import cn.Power.mod.mods.PLAYER.Derp;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
	...
	public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		if (Chams.isEnabled) {
			...
			chams = (Chams) ModManager.getModByClass(Chams.class);

			
			if (chams.isEnabled()) {
				if (chams.isvalid(entity)) {
					GL11.glEnable(32823);
		...
		if (true) {
			...
			try {
				...
				if (entity instanceof EntityPlayerSP && entityYaw != 0) {
					...
					float YAW = EventPreMotion.YAW;
					float PITCH = EventPreMotion.PITCH;
					float PREVYAW = EventPreMotion.PREVYAW;
					float PREVPITCH = EventPreMotion.PREVPITCH;

//					if ((ModManager.getModByClass(KillAura.class).isEnabled()
//							&& KillAura.Rotary_animation.getValueState() && KillAura.Target != null)
//							|| (ModManager.getModByClass(Scaffold.class).isEnabled())
//							|| (ModManager.getModByClass(Derp.class).isEnabled()
//									&& Derp.Rotary_animation.getValueState())
//							|| (BowAimbot.target != null && BowAimbot.shouldAim())) {
					
//					f = this.interpolateRotation(EventPreMotion.PREVYAW, EventPreMotion.YAW, partialTicks);
//					}
...
//					}
				...
				this.rotateCorpse(entity, f8-160f, f, partialTicks);
				...
				if (net.optifine.entity.model.CustomEntityModels.isActive()) {
				...
				if (this.renderOutlines) {
					...
					if (valid) {
						if (chams.isEnabled() && Chams.COLORED.getValueState()) {
							if (chams.isvalid(entity) && (!chams.team.getValueState() || !Teams.isOnSameTeam(entity))) {
								Minecraft mc = Minecraft.getMinecraft();
								int renderColor = ClientUtil.reAlpha(FlatColors.WHITE.c, 1.0F);
								...
								if (Character.toLowerCase(text.charAt(0)) == '\u00a7') {
									...
									if (colorCode < 16) {
										try {
											...
											renderColor = Colors.getColor((newColor >> 16), (newColor >> 8 & 0xFF),
													(newColor & 0xFF), 255);
		...
		if (true) {
			if (!this.canRenderName(entity)) {
				...
				if (entity instanceof EntityPlayer) {
					...
					if (ModManager.getModByClass(NameTag.class).isEnabled()
							|| ModManager.getModByClass(NameTag2D.class).isEnabled()
							|| ((E2D = (ESP2D) ModManager.getModByClass(ESP2D.class)).isEnabled()
									&& E2D.name.getValueState())) {
