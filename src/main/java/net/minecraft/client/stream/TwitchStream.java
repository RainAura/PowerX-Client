...
public class TwitchStream implements BroadcastController.BroadcastListener, ChatController.ChatListener,
			...
			Thread thread = new Thread("Twitch authenticator") {
				public void run() {
					try {
						...
						TwitchStream.LOGGER.error(TwitchStream.STREAM_MARKER, "Could not authenticate with twitch",
	...
	public void func_152911_a(Metadata p_152911_1_, long p_152911_2_) {
		if (this.isBroadcasting() && this.field_152957_i) {
			...
			if (!this.broadcastController.func_152840_a(p_152911_1_.func_152810_c(), i + p_152911_2_,
				...
				LOGGER.warn(STREAM_MARKER, "Couldn't send stream metadata action at {}: {}", i + p_152911_2_,
						p_152911_1_);
			} else {
	...
	public void func_176026_a(Metadata p_176026_1_, long p_176026_2_, long p_176026_4_) {
		if (this.isBroadcasting() && this.field_152957_i) {
			...
			if (j < 0L) {
				LOGGER.warn(STREAM_MARKER, "Could not send stream metadata sequence from {} to {}: {}", i + p_176026_2_,
						i + p_176026_4_, p_176026_1_);
			} else if (this.broadcastController.func_177947_a(p_176026_1_.func_152810_c(), i + p_176026_4_, j, s, s1)) {
				LOGGER.debug(STREAM_MARKER, "Sent stream metadata sequence from {} to {}: {}", i + p_176026_2_,
						i + p_176026_4_, p_176026_1_);
			} else {
				LOGGER.warn(STREAM_MARKER, "Half-sent stream metadata sequence from {} to {}: {}", i + p_176026_2_,
						i + p_176026_4_, p_176026_1_);
			}
		}
	}
	...
	public void func_152930_t() {
		...
		LOGGER.info(STREAM_MARKER, "Streaming at {}/{} at {} kbps to {}", videoparams.outputWidth,
				videoparams.outputHeight, videoparams.maxKbps, this.broadcastController.getIngestServer().serverUrl);
	...
	public void func_152897_a(ErrorCode p_152897_1_) {
		if (ErrorCode.succeeded(p_152897_1_)) {
			...
			LOGGER.warn(STREAM_MARKER, "Login attempt unsuccessful: {} (error code {})",
					ErrorCode.getString(p_152897_1_), p_152897_1_.getValue());
			this.loggedIn = false;
		}
	}
	...
	public void func_152894_a(StreamInfo p_152894_1_) {
		LOGGER.debug(STREAM_MARKER, "Stream info updated; {} viewers on stream ID {}", p_152894_1_.viewers,
				p_152894_1_.streamId);
	}
	...
	public void func_152893_b(ErrorCode p_152893_1_) {
		LOGGER.warn(STREAM_MARKER, "Issue submitting frame: {} (Error code {})", ErrorCode.getString(p_152893_1_),
				p_152893_1_.getValue());
		...
		for (ChatRawMessage chatrawmessage : p_180605_2_) {
			...
			if (this.func_176028_a(chatrawmessage.modes, chatrawmessage.subscriptions,
				...
				IChatComponent ichatcomponent1 = new ChatComponentTranslation(
						"chat.stream." + (chatrawmessage.action ? "emote" : "text"), this.twitchComponent,
						ichatcomponent, EnumChatFormatting.getTextWithoutFormattingCodes(chatrawmessage.message));
