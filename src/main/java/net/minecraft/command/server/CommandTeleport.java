		...
		if (args.length < 1) {
			...
			if (args.length != 1 && args.length != 2) {
				if (args.length < i + 3) {
					...
					CommandBase.CoordinateArg commandbase$coordinatearg1 = parseCoordinate(entity.posY, args[j++], 0, 0,
							false);
					CommandBase.CoordinateArg commandbase$coordinatearg2 = parseCoordinate(entity.posZ, args[j++],
							true);
					...
					CommandBase.CoordinateArg commandbase$coordinatearg4 = parseCoordinate(
							(double) entity.rotationPitch, args.length > j ? args[j] : "~", false);
