...
public class CrashReport {
	...
	private void populateEnvironment() {
		...
		this.theReportCategory.addCrashSectionCallable("Memory", new Callable<String>() {
			...
			public String call() {
				...
				return k + " bytes (" + j2 + " MB) / " + j + " bytes (" + i2 + " MB) up to " + i + " bytes (" + l
						+ " MB)";
			}
		});
	...
	public void getSectionsInStringBuilder(final StringBuilder builder) {
		if ((this.stacktrace == null || this.stacktrace.length <= 0) && this.crashReportSections.size() > 0) {
			this.stacktrace = (StackTraceElement[]) ArrayUtils
					.subarray((Object[]) this.crashReportSections.get(0).getStackTrace(), 0, 1);
		}
	...
	public CrashReportCategory makeCategoryDepth(final String categoryName, final int stacktraceLength) {
		...
		if (this.firstCategoryInCrashReport) {
			...
			if (i > 0 && !this.crashReportSections.isEmpty()) {
				final CrashReportCategory crashreportcategory2 = this.crashReportSections
				...
				System.arraycopy(astacktraceelement, 0, this.stacktrace = new StackTraceElement[j], 0,
	...
	private static String getWittyComment() {
		final String[] astring = { "Who set us up the TNT?",
