...
public abstract class Entity implements ICommandSender {
	...
	public void onEntityUpdate() {
		...
		if (this.equals(Minecraft.thePlayer)){
			handleWaterMovement();
		}
	...
	public void moveEntity(double x, double y, double z) {
		if (this.noClip) {
			...
			double d4 = y;
			double d5 = z;
			boolean flag = this.onGround && this.isSneaking() && this instanceof EntityPlayer && !ModManager.getModByClass(Scaffold.class).isEnabled();
			...
			EventStep stepEvent = new EventStep(true, this.stepHeight);
			...
			if (stepEvent.getStepHeight() > 0.0F && flag1 && (d3 != x || d5 != z)) {
				...
				EventManager.call(new EventStep(false, 1D + y));
			}
	...
	public boolean handleWaterMovement() {
		if (this.worldObj.handleMaterialAcceleration(
				this.getEntityBoundingBox().expand(0.0D, (double) -0.4F, 0.0D).contract(0.001D, 0.001D, 0.001D),
				Material.water, this)) {
	...
	protected void resetHeight() {
		float f = MathHelper.sqrt_double(this.motionX * this.motionX * (double) 0.2F + this.motionY * this.motionY
	...
	public boolean isInLava() {
		return this.worldObj.isMaterialInBB(
				this.getEntityBoundingBox().expand((double) -0.1F, (double) -0.4F, (double) -0.1F), Material.lava);
	}
...
//		return 15f;
	...
	public boolean isEntityInsideOpaqueBlock() {
		...
		if(viewclip == null)
			viewclip = ModManager.getModByClass(ViewClip.class);
...
//	}
	...
	public float getCollisionBorderSize()
	{
		return (ViaLoadingBase.getInstance().getTargetVersion().getIndex() >= 35) ? 0.1f : 0.0f;
	}
	...
	public String toString() {
		return String.format("%s['%s'/%d, l='%s', x=%.2f, y=%.2f, z=%.2f]", this.getClass().getSimpleName(),
				this.getName(), this.entityId,
				this.worldObj == null ? "~NULL~" : this.worldObj.getWorldInfo().getWorldName(), this.posX, this.posY,
				this.posZ);
	}
