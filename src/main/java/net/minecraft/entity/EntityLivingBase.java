...
public abstract class EntityLivingBase extends Entity {
	...
	private static final AttributeModifier sprintingSpeedBoostModifier = (new AttributeModifier(
			sprintingSpeedBoostModifierUUID, "Sprinting speed boost", (double) 0.3F, 2)).setSaved(false);
		...
		if (!this.worldObj.isRemote && this.fallDistance > 3.0F && onGroundIn) {
			...
			if (block.getMaterial() != Material.air) {
				...
				((WorldServer) this.worldObj).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY,
						this.posZ, i, 0.0D, 0.0D, 0.0D, (double) 0.15F, Block.getStateId(iblockstate));
			}
		}
			...
			if (this.getHealth() <= 0.0F) {
				...
				if (flag) {
					...
					if (entity != null) {
						...
						this.attackedAtYaw = (float) (MathHelper.atan2(d0, d1) * 180.0D / (double) (float) Math.PI
	...
	private int getArmSwingAnimationEnd() {
		Animation Anim = (Animation) ModManager.getModByClass(Animation.class);
		if (Minecraft.getMinecraft().thePlayer == this && Anim.isEnabled() && Anim.DigSpeed.getValueState()) {
			return Anim.SwingSpeed.getValueState().intValue();
		}
		if (this.isPotionActive(Potion.digSpeed)) {
			return 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1;
		} else {
			return this.isPotionActive(Potion.digSlowdown)
	...
	protected void jump() {
		...
		if (this instanceof EntityPlayerSP) {
			EventJump ej = new EventJump();
			EventManager.call(ej);

			double ymot = this.isPotionActive(Potion.jump)
			...
			if (isSprinting()) {
				...
				float speed = 0.2f;
				motionX -= MathHelper.sin(f) * speed;
				motionZ += MathHelper.cos(f) * speed;
			}
			isAirBorne = true;
	...
	public void moveEntityWithHeading(float strafe, float forward) {
		if (this.isServerWorld() || this instanceof EntityHorse) {
			if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
				if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
					...
					if (this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.motionX,
							this.motionY + (double) 0.6F - this.posY + d1, this.motionZ)) {
				...
				if (this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.motionX,
						this.motionY + (double) 0.6F - this.posY + d0, this.motionZ)) {
					this.motionY = (double) 0.3F;
				}
			}
		}
	...
	public void onLivingUpdate() {
		...
		if (Math.abs(this.motionX) < ((ViaLoadingBase.getInstance().getTargetVersion().getIndex() >= 35) ? 0.005 : 0.003)) {
			this.motionX = 0.0;
		}
		if (Math.abs(this.motionY) < ((ViaLoadingBase.getInstance().getTargetVersion().getIndex() >= 35) ? 0.005 : 0.003)) {
			this.motionY = 0.0;
		}
		if (Math.abs(this.motionZ) < ((ViaLoadingBase.getInstance().getTargetVersion().getIndex() >= 35) ? 0.005 : 0.003)) {
			this.motionZ = 0.0;
		}
	...
	protected void collideWithNearbyEntities() {
		List<Entity> list = this.worldObj.getEntitiesInAABBexcluding(this,
				this.getEntityBoundingBox().expand((double) 0.2F, 0.0D, (double) 0.2F),
