...
public class EntityWither extends EntityMob implements IBossDisplayData, IRangedAttackMob {
	...
	public void onLivingUpdate() {
		...
		for (int l = 0; l < 3; ++l) {
			...
			this.worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d10 + this.rand.nextGaussian() * (double) 0.3F,
					d2 + this.rand.nextGaussian() * (double) 0.3F, d4 + this.rand.nextGaussian() * (double) 0.3F, 0.0D,
					0.0D, 0.0D);

			if (flag && this.worldObj.rand.nextInt(4) == 0) {
				this.worldObj.spawnParticle(EnumParticleTypes.SPELL_MOB, d10 + this.rand.nextGaussian() * (double) 0.3F,
						d2 + this.rand.nextGaussian() * (double) 0.3F, d4 + this.rand.nextGaussian() * (double) 0.3F,
		...
		if (this.getInvulTime() > 0) {
			for (int i1 = 0; i1 < 3; ++i1) {
				this.worldObj.spawnParticle(EnumParticleTypes.SPELL_MOB, this.posX + this.rand.nextGaussian() * 1.0D,
						...
						this.posZ + this.rand.nextGaussian() * 1.0D, (double) 0.7F, (double) 0.7F, (double) 0.9F);
			}
		}
	}
	...
	public boolean isArmored() {
		return this.getHealth() <= this.getMaxHealth() / 2.0F && !this.isInvisible();
	}
