		...
		if (id == 15) {
			for (int i = 0; i < this.rand.nextInt(35) + 10; ++i) {
				this.worldObj.spawnParticle(EnumParticleTypes.SPELL_WITCH,
						this.posX + this.rand.nextGaussian() * (double) 0.13F,
						this.getEntityBoundingBox().maxY + 0.5D + this.rand.nextGaussian() * (double) 0.13F,
						this.posZ + this.rand.nextGaussian() * (double) 0.13F, 0.0D, 0.0D, 0.0D);
			}
		} else {
