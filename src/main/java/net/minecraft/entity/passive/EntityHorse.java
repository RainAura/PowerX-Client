...
public class EntityHorse extends EntityAnimal implements IInvBasic {
	...
	public EntityHorse(World worldIn) {
		super(worldIn);
		if(ModManager.getModByClass(HorseFounder.class).isEnabled())
			((HorseFounder)ModManager.getModByClass(HorseFounder.class)).PreformActions();
	...
	public void setHorseTamed(boolean tamed) {
		...
		if(tamed && ModManager.getModByClass(HorseFounder.class).isEnabled())
			ClientUtil.sendChatMessage("Set Horse tamed", ChatType.INFO);
	}
...
//			}
			...
			if (this.jumpPower > 0.0F && !this.isHorseJumping() && this.onGround) {
				...
				if (forward > 0.0F) {
					...
					f = MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F);
					f1 = MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F);
					this.motionX += (double) (-0.4F * f * this.jumpPower);
					this.motionZ += (double) (0.4F * f1 * this.jumpPower);
			...
			if (ViaLoadingBase.getInstance().getTargetVersion().getOriginalVersion() > 47 && this.worldObj.isRemote && ((EntityLivingBase) this.riddenByEntity) == Minecraft.thePlayer) {
				this.setAIMoveSpeed(
						(float) this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).getAttributeValue() * 2);
				super.moveEntityWithHeading(strafe, forward);
				
				PacketWrapper exloit = PacketWrapper.create(0x10, null, Minecraft.getNetHandler().getNetworkManager().user);
				exloit.write(Type.DOUBLE, ((EntityLivingBase) this.riddenByEntity).ridingEntity.posX );
				exloit.write(Type.DOUBLE, ((EntityLivingBase) this.riddenByEntity).ridingEntity.posY + 0.0001);
				exloit.write(Type.DOUBLE, ((EntityLivingBase) this.riddenByEntity).ridingEntity.posZ);
				exloit.write(Type.FLOAT, (float)1.0);
				exloit.write(Type.FLOAT, (float)Minecraft.thePlayer.rotationPitch / 2);
				try {
