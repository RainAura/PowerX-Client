						...
						if (i > 0) {
							targetEntity.addVelocity(
									...
									0.1D, (double) (MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F)
											* (float) i * 0.5F));
							if (ModManager.getModByClass(KeepSprint.class).isEnabled()
									|| ModManager.getModByClass(KillAura.class).isEnabled()) {
								return;
							}
							this.motionX *= 0.6D;
							this.motionZ *= 0.6D;
...
//	}
	...
	public void jump() {
		super.jump();
		final float f = (ViaLoadingBase.getInstance().getTargetVersion().getIndex() >= 35) ? 0.8f : 0.2f;
		final float f2 = (ViaLoadingBase.getInstance().getTargetVersion().getIndex() >= 35) ? 0.2f : 0.05f;
		this.triggerAchievement(StatList.jumpStat);
		if (this.isSprinting()) {
	...
	public IChatComponent oldChatComponent;
	...
	public IChatComponent getDisplayName() {
		...
		if (oldTeam == null || this.getTeam() == null || !oldTeam.isSameTeam(this.getTeam()) || !oldName.equals(this.getName())) {
			
			oldTeam = this.getTeam();
			oldName = this.getName();
			
			IChatComponent ichatcomponent = new ChatComponentText(ScorePlayerTeam.formatPlayerName(oldTeam, oldName));
			...
			oldChatComponent = ichatcomponent;
